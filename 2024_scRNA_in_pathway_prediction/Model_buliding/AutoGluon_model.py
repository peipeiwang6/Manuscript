...
def F1score_Calculate(gene_class_df, txt, AnyData, AnyResult, raw_name, unknown_name):
    ...
    if 'Autogluon' in txt:
        ...
        for run in range(10):
            ...
            multi_predictor.fit(Real_gene_df.iloc[:,1:], num_bag_folds=5)
            for ll in label:
        ...
        for n in range(10):
            overall_score_list = []
            Real_gene_df = Auto_Input_df.sample(frac=1, random_state=n).reset_index(drop=True)
            save_path = 'Result_All/Overall_Real_model_file_%s/Models-predictEducationClass%s' % (txt.split('.')[0], n)
            problem_types = ['binary'] * 14
            multi_predictor = MultilabelPredictor(labels=label, problem_types=problem_types, eval_metrics=['f1'] * 14, path=save_path)
            multi_predictor.fit(Real_gene_df.iloc[:,1:], num_bag_folds=5)
            train_pred = multi_predictor.predict(Real_gene_df.iloc[:, 1:])
            y_train = Real_gene_df[label]
            train_f1 = f1_score(y_train, train_pred, average='micro')
            overall_score_list.append(train_f1)
            global_train.append(train_f1)
            gene_test_multilabel = pd.merge(gene_test_matrix_df.loc[:, 'Gene'], class_csr_matrix_df, on="Gene", how="left")
            test_multilabel = np.array(gene_test_multilabel[[x for x in list(gene_test_multilabel)[1:]]] == 1)
            X_test, y_test = gene_test_matrix_df.iloc[:, 1:], test_multilabel
            test_pred = multi_predictor.predict(X_test)
            test_scores = f1_score(y_test, test_pred, average='micro')
            global_test.append(test_scores)
        global_train_df = pd.DataFrame(global_train, columns=["Global_F1_Score"])
        global_train_df.to_csv(Global_train_name, index = False)
        global_test_df = pd.DataFrame(global_test, columns=["Global_F1_Score"])
        global_test_df.to_csv(Global_test_name, index = False)
...
def Bg_Value(gene_class_df, txt, AnyData, AnyResult, raw_name, raw_test):
    ...
    if 'Autogluon' in txt:
        ...
        for num in range(0, 100):
            ...
            multi_predictor = MultilabelPredictor(labels=label, problem_types=problem_types, eval_metrics=['f1'] * 14, path=save_path)
            multi_predictor.fit(Bg_gene_df.iloc[:, 1:], num_bag_folds=5)
            for ll in label:
                score = pd.DataFrame(multi_predictor.get_predictor(ll).leaderboard()).score_val.to_list()[0]
        ...
        for m in range(0, 100):
            save_path = 'Result_All/Overall_BG_test_model_file_%s/agModels-predictEducationClass%s' % (txt.split('.')[0], m)
            Bg_gene_df = pd.concat([gene_train_matrix_df.sample(frac=1, random_state=m).reset_index(drop=True), gene_train_multilabel.drop(['Gene'], axis=1)], axis=1)
            multi_predictor = MultilabelPredictor(labels=label, problem_types=problem_types, eval_metrics=['f1'] * 14, path=save_path)
            multi_predictor.fit(Bg_gene_df.iloc[:, 1:], num_bag_folds=5)
            bg_pred = multi_predictor.predict(Bg_gene_df.iloc[:, 1:])
            y_test = Bg_gene_df[label]
            global_f1 = f1_score(y_test, bg_pred, average='micro')
            overall_bg_train.append(global_f1)
            gene_test_multilabel = pd.merge(gene_test_matrix_df.loc[:, 'Gene'], class_csr_matrix_df, on="Gene", how="left")
            test_multilabel = np.array(gene_test_multilabel[[x for x in list(gene_test_multilabel)[1:]]] == 1)
            X_test, y_test = gene_test_matrix_df.iloc[:, 1:], test_multilabel
            test_pred = multi_predictor.predict(X_test)
            test_scores = f1_score(y_test, test_pred, average='micro')
            overall_bg_test.append(test_scores)
        overall_bg_train_df = pd.DataFrame(overall_bg_train, columns=["Global_F1_Score"])
        overall_bg_train_df.to_csv(Global_Bg_train, index = False)
        overall_bg_test_df = pd.DataFrame(overall_bg_test, columns=["Global_F1_Score"])
